...
@section('page-scripts')
  ...
  <script src="{{ asset('web/js/based/datatables/itinerary-datatable.js') }}"></script>
@endsection
...
@section('page-custom-scripts')
  ...
  <script>
    ...
    let urlTable = "{{ route('admin.itinerary.datatable') }}";
    let createUrl = "{{ route('admin.itinerary.create') }}";
    let editUrl = "{{ route('admin.itinerary.edit', ':id') }}";
  </script>
  <script type="text/javascript">
    $(document).ready(function () {
      ...
      $("#setup-button").click(function (e) {
        e.preventDefault();
        $.ajax({
          url: "{{ route('admin.itinerary.create') }}",
          type: "GET",
      ...
      $('.btn-edit-modal').click(function (e) {
        ...
        let url_edit = "{{ route('admin.itinerary.edit', ':id') }}";
          ...
          <div class="d-flex flex-stack flex-wrap mb-5">
            ...
            <div class="d-flex align-items-center position-relative my-1 mb-2 mb-md-0">
              ...
              <input type="text" data-kt-user-table-filter="search"
                     class="form-control form-control-solid w-250px ps-15" placeholder="Search Kegiatan"/>
            </div>
            <!--end::Search-->
            <!--begin::Toolbar-->
            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                  ...
                  <div class="mb-10">
                    ...
                    <select class="form-select form-select-solid fw-bold" data-kt-select2="true"
                      ...
                      @foreach(FacilityType::cases() as $type)
                        <option
                          value="{{ $type->keyValue() }}">{{ \Illuminate\Support\Str::ucfirst($type->keyValue()) }}</option>
                      @endforeach
                    </select>
                  </div>
                  <!--end::Input group-->
              ...
              @can("create {$current_page}")
                <button type="button" class="btn btn-primary" id="create-new" data-bs-toggle="tooltip"
                        title="Setup Kegiatan">
                  <!--begin::Svg Icon | path: icons/duotune/arrows/arr075.svg-->
                  <span class="svg-icon svg-icon-2">
                  <i class="fa-solid fa-plus"></i>
                </span>
                  <!--end::Svg Icon-->Buat Kegiatan
                </button>
      ...
      <div class="table-responsive">
        ...
        <table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
          ...
          <tr class="fw-bold text-muted">
            ...
            <th class="min-w-150px">Nama Aktifitas</th>
            <th class="min-w-140px">Detail Aktifitas</th>
          ...
          @forelse ($itineraries as $itinerary)
            <tr>
              ...
              <td class="text-dark fw-bold text-hover-primary fs-6">{{ $itinerary->activity }}</td>
              <td class="text-dark fw-bold text-hover-primary fs-6">

                @if (isset($itinerary->detail))
                  {{ mb_strimwidth($itinerary->detail, 0, 20, '...') }}
                  @if (strlen($itinerary->detail) > 20)
                    <i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="tooltip"
                       title="{{ $itinerary->detail }}"></i>
                  @endif
                @else
                  -
                @endif
              ...
              <!--begin::Action=-->
              <td class="text-end">
                <a href="#" data-id="{{ $itinerary->hash }}" data-bs-toggle="tooltip"
                ...
                <a data-bs-toggle="tooltip" title="Hapus Aktifitas" onclick="$('#delete').submit()"
                   class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1 text-muted">
                    <i class="fa-solid fa-trash"></i>
                </a>

                <!--begin::Menu item-->
                <form action="{{ route('admin.itinerary.destroy', $itinerary->hash) }}" hidden
