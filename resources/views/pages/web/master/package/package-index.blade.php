  ...
  <script type="text/javascript">
    $(document).ready(function () {
      ...
      $("#setup-button").click(function (e) {
        e.preventDefault();
        $.ajax({
          url: "{{ route('admin.master.package.create') }}",
          type: "GET",
      ...
      $('.btn-edit-modal').click(function (e) {
        ...
        let url_edit = "{{ route('admin.master.package.edit', ':id') }}";
      ...
      $('.btn-itinerary-setup').click(function (e) {
        e.preventDefault();
        var id = $(this).attr("data-id");
        let urlSetupItinerary = "{{ route('admin.master.package.itinerary-setup.create', ':id') }}";
        urlSetupItinerary = urlSetupItinerary.replace(":id", id);

        $.ajax({
          url: urlSetupItinerary,
          type: "GET",
          success: function (data) {
            ...
            let dateTimePicker = $('.time-picker');
            ...
            $(".add-itinerary-activity").click(function(){
              let getItineraryId = $(this).attr("data-itinerary");
              let addItineraryTo = $(this).attr("data-add-itinerary");

              let clone = $("#"+getItineraryId).clone();
              clone.removeAttr('hidden')
              clone.find("span").remove();
              clone.find("select").select2();

              var uiDate = clone.find(".time-picker");
              if(uiDate){
                flatpickr(uiDate, {
                  enableTime: true,
                  noCalendar: true,
              ...
              clone.appendTo("#"+addItineraryTo);

              $(clone).find(".btn-remove-itinerary").click(function (e) {
                e.preventDefault();
                $(clone).remove().end().appendTo("#"+addItineraryTo)
              });
        ...
        <table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
          ...
          @forelse ($packages as $package)
            <tr>
              ...
              <td class="text-dark fw-bold text-hover-primary fs-6">{{ $package->status }}
                <span
                  class="text-muted fw-semibold text-muted d-block fs-7">{{ $package->is_publish ? 'Published' : 'Unpublished' }}</span>
              </td>
              ...
              <td class="text-end">
                ...
                <div
                  ...
                  @if ($package->jamaah_count < 1)
                    <!--begin::Menu item-->
                    <form action="{{ route('admin.master.package.destroy', $package->id) }}"
