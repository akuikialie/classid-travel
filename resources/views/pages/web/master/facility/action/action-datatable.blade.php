  ...
  @can("update {$current_page}")
    ...
    <div class="menu-item px-3" data-kt-menu-trigger="hover"
      ...
      <div class="menu-sub menu-sub-dropdown w-175px py-4 px-4 ">
        <form method="post" data-kt-form-id="change-status-{{ $facility->hash }}" action="{{ route('admin.facility.change-status', $facility->hash) }}">
          @csrf
          <a href="#" class="menu-link px-3 change-status {{ $facility->is_active ? 'active' : '' }}"
             data-id="{{ $facility->hash }}" data-status="1">
        <span class="menu-bullet">
          ...
          <!--begin::Menu item-->
          <a href="#" class="menu-link px-3 change-status {{ $facility->is_active ? '' : 'active' }}"
             data-id="{{ $facility->hash }}" data-status="0">
        <span class="menu-bullet">
          <span class="bullet bullet-dot"></span>
        </span>
  ...
  @can("delete {$current_page}")
    <div class="menu-item px-3 text-nowrap">
      <form action="{{ route('admin.facility.destroy', $facility->hash) }}" method="post"
        ...
        @csrf
        @method('DELETE')
        <a class="menu-link px-3 btn-delete" data-id="{{ $facility->hash }}"
           data-bs-toggle="tooltip" title="Hapus Fasilitas">
          <span class="badge badge-light-danger"> Hapus Fasilitas</span>
        </a>
      </form>
    </div>
  @endcan
