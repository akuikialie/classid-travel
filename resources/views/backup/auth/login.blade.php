...
@section('content')
  <div class="px-20 px-md-10 pt-10">
    ...
    <form name="form" method="post" action="{{ route('authentication') }}" class="form w-100" id="kt_sign_in_form">
      @csrf
      ...
      @endif --}}
      <div class="fv-row mb-10 {{ $errors->has('email') || $errors->has('username') ?:' has-danger' }}">
        <label class="form-label fs-6 fw-bolder text-dark">Email / Username</label>
        <input class="form-control form-control-lg form-control-solid" type="text" name="email-username" tabindex="1" value="{{ old('email-username') }}" placeholder="Email / Username" required autofocus>
      </div>
